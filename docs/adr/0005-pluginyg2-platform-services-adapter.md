...
- Use one narrow project-owned lifecycle diagnostics channel over the existing PluginYG2 `YGSendMessage` component when the vendor callback surface cannot carry evidence back to Unity. The channel has two typed messages, `RobotArenaPlatformState` and `RobotArenaPlatformPause`, and may report the terminal SDK state, actual capabilities, and an explicit Yandex pause/resume origin. It is not a second initializer, a general-purpose bridge, or a gameplay API.
